...
def parse_line(line, af_limit = 0.0001):
    ...
    # Parse the INFO field to extract key-value pairs.
    #   We know the ; because of instructions "Semi-colon (;) separated key-value pairs"
    info_pairs = info.split(';')
    
    info_dict = {}
    for pair in info_pairs:
      if '=' in pair: # ignore entries with no '='
        key, value = pair.split('=', 1)  # Split only on first '=' in case value contains '='
        info_dict[key] = value
    
    # Exract clndn. If it isn't present, skip the line.
    if not "CLNDN" in info_dict.keys():
        return []
    clndn = info_dict["CLNDN"]
    
    # Exract af_exac and convert to float.
    # If it isn't present, or can't be converted, skip the line.
    try:
    ...
    if af_exac < af_limit:
        ...
        # Return the valid diseases in a list.
        return filtered_diseases
    else:
      return [] # If variant is not rare, return empty list

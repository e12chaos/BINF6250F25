...
def find_match(query, reference):
    ...
    suffix_positions = suffix_array(reference + '$')  # Add $ terminator
    bwt = BWT_from_suffix_array(reference + '$', suffix_positions)
    count = cal_count(reference + '$')
    occur = cal_occur(bwt)
    
    # Step 2: Initialize range
    lower = 0
    upper = len(reference)
    
    # Step 3: Loop backwards through query
    ...
    # Step 4: Collect suffix positions in the final range
    # Step 4: Collect suffix positions in the final range
    matches = suffix_positions[lower:upper]
    # What array contains the suffix positions?

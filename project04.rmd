...
class DeBruijnGraph():
    ...
    def eulerian_walk(self, node, seed=None):
        ...
        ['est', ...]
        '''
        if len(self.graph[node]) == 0:#Is the current node out of edges? if you reach a node with no more edges you're done. You've hit a dead end, so you return just that final node.
            return [node]
        else:
            if seed:
                random.seed(seed)
            #next_node = "fool "
            next_node = self.graph[node][0] #Randomly picks one of the nodes that the current node connects to
